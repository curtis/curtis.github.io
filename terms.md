---
layout: page
nav: false
title: Terms
description: The basics of using this site.
---

This is a personal blog, not a legal minefield. Here's the short version.

### Content

Everything I write here reflects my own opinions and experiences. Posts about code, tools, or techniques are shared in good faith, but I can't guarantee they'll work perfectly in your situation. Use your judgment.

Older posts may reference outdated tools, libraries, or practices. Tech moves fast -- something that worked in 2007 might not be the best approach today.

### Comments

If you leave comments, keep it civil. I reserve the right to remove anything that's spam, abusive, or off-topic.

### Links

I link to other sites. I'm not responsible for their content or practices. If a link is broken or points somewhere sketchy, [let me know](mailto:curtm95@gmail.com).

### Copyright

The writing and code examples on this blog are mine unless otherwise noted. You're welcome to reference and link to anything here. If you want to republish a full post, just ask.

### Changes

I might update these terms if something changes. This is a blog, not a SaaS product, so don't expect frequent revisions.

### Questions

Email me at [curtm95@gmail.com](mailto:curtm95@gmail.com).
